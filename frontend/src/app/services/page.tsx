    ...
    <article className={`rounded-[28px] border p-6 shadow-[0_20px_60px_rgba(61,84,128,0.06)] ${
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-3 text-[26px] font-bold tracking-tight text-[#0d1424]">{product.name}</h2>
        </div>
      ...
      {product.headline ? <p className="mt-4 text-[15px] leading-7 text-[#435774]">{product.headline}</p> : null}
      <p className="mt-4 text-[14px] leading-6 text-[#5f6f88]">{product.description}</p>
      ...
      <div className="mt-8">
        ...
        >
          {product.cta_label || (product.active ? "View service" : "Coming soon")}
        </Link>
      </div>
    </article>
  );
...
export default function ServicesPage() {
  ...
  const upcomingProducts = products.filter((product) => !product.active);
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(91,141,238,0.14),_transparent_30%),linear-gradient(180deg,#edf3f9_0%,#f6f9fd_100%)] px-6 py-10">
      <div className="mx-auto max-w-6xl">
        <div className="rounded-[32px] border border-white/80 bg-white/78 p-8 shadow-[0_28px_80px_rgba(61,84,128,0.08)] backdrop-blur md:p-10">
          <div className="text-[12px] font-semibold uppercase tracking-[0.22em] text-[#7b8ba5]">Guardian services</div>
          <div className="mt-3 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <p className="mt-4 max-w-3xl text-[16px] leading-7 text-[#556480]">
                The catalog is now config-driven. Form 8843 is live; the rest of the rail is staged so checkout and intake can be layered on product by product instead of rebuilding the storefront each time.
              </p>
            </div>
            <Link
              href="/form-8843"
        ...
        <section className="mt-8">
          <div className="mb-4 text-[12px] font-semibold uppercase tracking-[0.18em] text-[#6d7c95]">Live now</div>
        ...
        {upcomingProducts.length ? (
          <section className="mt-10">
            <div className="mb-4 text-[12px] font-semibold uppercase tracking-[0.18em] text-[#6d7c95]">Up next</div>
            <div className="grid gap-6 lg:grid-cols-2">
