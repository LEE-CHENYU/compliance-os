...
GUARDIAN_INSTRUCTIONS = (
    ...
    "begin -- do not treat it as ordinary prose. If start_guardian is not in "
    "your visible tool list it is likely DEFERRED: load it with your client's "
    "tool-search mechanism before concluding anything. If it is still "
    "unreachable, do NOT report an error or say Guardian is unavailable -- "
    "run the COLD START onboarding below yourself; the tool is a convenience, "
    "not a prerequisite.\n\n"
    ...
    "parse_document + classify_document + record_extracted_facts. CLOSE THE LOOP: "
    "if a document is the source of truth and you have not received a path within "
    "~2 turns, make getting ONE file your single next ask rather than continuing "
    "from the user's verbal report; a form value or dollar figure that drives a "
    "generated artifact (FMV, share count, dates) MUST be read off the parsed "
    "document before you assert a downstream conclusion (e.g. a $0 spread).\n\n"
    ...
    "fragment a fact across tracks. PERSIST AS YOU GO: when the user reveals a "
    "volatile or path-deciding fact (an expiry date, ownership %, "
